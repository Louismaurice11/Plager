...
class CVisitor(ParseTreeVisitor):
    ...
    def setUserDefinedTypes(self,typedefs):
    ...
    def visitTerminal(self, node):
        ...
        # If the identifier is a user defined type
        if node.getText() in self.typedefs:
            # Don't replace it
            return

        line_number = node.symbol.line

        # Otherwise replace the identifier names
        if node.symbol.type == CLexer.Identifier:
            # Create a new token for the variable
            filteredToken = CommonToken(type=CLexer.Identifier,channel=node.symbol.channel,
                                        start=node.symbol.start, stop=node.symbol.stop)

            # Set it equal to the same name for every function, class and variable name (the identifier)
            filteredToken.text = "<IDENTIFIER>"

            # Set the line number of the new token
            filteredToken.line = node.symbol.line

            # set the token source to be that of the original variable token
            filteredToken.tokenSource = node.symbol.getTokenSource()

            # Set the start and stop indexes of the new token to match the skipped identifier
            filteredToken.start = node.symbol.start
            filteredToken.stop = node.symbol.stop

            # Replace the original token with the new token
            node.symbol = filteredToken


        # Always return the modified token (or the original token for non-identifiers)
        return node
    ...
    def visitIdentifierList(self, ctx: CParser.IdentifierListContext):
        ...
        for identifier in ctx.Identifier():
            ...
            mIdentifier = TerminalNodeImpl(CommonToken(CParser.Identifier, text))

            ctx.removeLastChild()
            ctx.addChild(mIdentifier)
            identifiers.append(mIdentifier)
    ...
    def visitTypedefName(self, ctx: CParser.TypedefNameContext):
        print("----------------" + ctx.getText() + "------------")
        self.typedefs.add(ctx.Identifier().getText())
        return self.visitChildren(ctx)
